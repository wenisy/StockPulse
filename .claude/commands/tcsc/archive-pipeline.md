...
Step 3: MR/PR 存在性检查        → 反问用户确认 MR 已创建
Step 4: openspec archive 调起   → 真正执行归档
Step 5: INDEX 语义化更新        → AI 读 specs/ 与 archive/ 后生成两个 INDEX.md
```
...
echo "[INFO] 远端仓库: $REMOTE"
```

随后 AI MUST 用 **AskUserQuestion tool** 反问用户:

```
是否已为本特性分支(<CURRENT_BRANCH>)创建合并请求(MR/PR)?
- 是:已创建,继续归档
- 否:需先创建 MR,中止本次归档
```

- 用户回答"否" → 阻止流程,中止归档,提示用户先创建 MR
- 用户回答"是" → 进入 Step 4

**注**:Step 3 是反问而非自动检测,因为不同代码托管平台(GitLab / GitHub / Gerrit / 工蜂)的 MR/PR API 差异大,反问比统一检测更稳。如果你的项目环境支持自动检测(如有 `gh` CLI 或工蜂 MCP 等工具可用),可以在反问前先尝试自动查询给用户参考。

---

## Step 4:调起 `openspec archive`
...
### Step 5-1:更新 `openspec/specs/INDEX.md`
...
### Step 5-2:更新 `openspec/changes/archive/INDEX.md`
...
本命令的全部行为 SHALL NOT 依赖 `openspec/config.yaml` 中的任何字段。即使项目没有装 `openspec-installer`、即使 `openspec/config.yaml` 缺失、即使 bridge rule 不存在,本命令仍可被用户主动调起并完成 5 步流程(前提仅为 `openspec` CLI 已安装且当前在 git 工作树内)。

如果 `openspec` CLI 未安装,Step 4 会返回错误,本命令会在该步终止——这是 OpenSpec CLI 的边界,不属于本命令的失败。
...
- Step 4 的 `openspec archive` 会报错"not in an openspec project"
- 本命令 MUST 输出:
