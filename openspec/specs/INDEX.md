...
| [portfolio-codebase-layout](./portfolio-codebase-layout/spec.md) | src/lib/portfolio/ 纯函数库目录结构契约、lib 层依赖方向约束、hooks 必须通过 lib 完成领域计算、聚合 hook 模式约定、单文件 300 行软约束 | 7 |
...
| [hooks-structure](./hooks-structure/spec.md) | 单职责 hook 边界定义、聚合 hook 向下兼容策略、行数约束、每个子 hook 必须有测试 | 4 |
| [component-structure](./component-structure/spec.md) | 大组件重构前必须先有集成测试、重构后组件 ≤ 300 行、组件拆分边界（主组件/子组件/业务 hook/API hook）、forwardRef API 兼容、集成测试 mock 边界 | 5 |
