...
import { computePortfolioValue, computeReturnRate, computeAllocation, computeYearlyGrowth } from '@/lib/portfolio/portfolio-metrics';

describe('数值边界保护', () => {
  describe('computeReturnRate 边界', () => {
    it('totalInvestment=0 返回 0（早返回保护）', () => {
      expect(computeReturnRate(10000, 0)).toBe(0);
    });

    it('totalInvestment 为负数返回 0', () => {
      expect(computeReturnRate(10000, -5000)).toBe(0);
    });

    it('portfolioValue=NaN 时不抛但返回 NaN（输入错由调用方负责）', () => {
      const result = computeReturnRate(NaN, 1000);
      expect(typeof result).toBe('number');
      // 不要求结果有限，但不能抛异常
    });

    it('totalInvestment=Infinity 时不抛异常（已知会返回 NaN，文档化）', () => {
      // 实际场景里 Infinity 不会发生（数据来自累加），留此用例防御未来回归
      expect(() => computeReturnRate(1000, Infinity)).not.toThrow();
    });
  });

  describe('computePortfolioValue 边界', () => {
    it('含 price=NaN 的 stocks 不抛', () => {
      expect(() => computePortfolioValue(
        [{ name: 'X', shares: 100, price: NaN, costPrice: 0, id: 'x' }],
        1000,
      )).not.toThrow();
    });

    it('cashBalance 为负数 → 应正常累加（用户透支场景）', () => {
      expect(computePortfolioValue([], -5000)).toBe(-5000);
    });

    it('含 shares=Infinity 的 stocks 不抛', () => {
      expect(() => computePortfolioValue(
        [{ name: 'X', shares: Infinity, price: 100, costPrice: 0, id: 'x' }],
        0,
      )).not.toThrow();
    });
  });

  describe('computeAllocation 边界', () => {
    it('totalValue=0 时所有占比返回 0', () => {
      const result = computeAllocation([], 0);
      expect(result['现金']).toBe(0);
    });

    it('totalValue<0 时所有占比返回 0（保护）', () => {
      const result = computeAllocation([], -1000);
      expect(result['现金']).toBe(0);
    });

    it('所有 stocks 全部 shares=0 → 各 stock 占比 0，现金 100', () => {
      const stocks = [
        { name: 'A', shares: 0, price: 100, costPrice: 0, id: 'a' },
        { name: 'B', shares: 0, price: 200, costPrice: 0, id: 'b' },
      ];
      const result = computeAllocation(stocks, 5000);
      expect(result['A']).toBe(0);
      expect(result['B']).toBe(0);
      expect(result['现金']).toBe(100);
    });
  });

  describe('computeYearlyGrowth 边界', () => {
    it('lastYearValue=0 返回 0（早返回）', () => {
      expect(computeYearlyGrowth(0, 1000)).toBe(0);
    });

    it('lastYearValue 为负 → 返回 0', () => {
      expect(computeYearlyGrowth(-100, 100)).toBe(0);
    });

    it('两年同值 → 返回 0', () => {
      expect(computeYearlyGrowth(1000, 1000)).toBe(0);
    });
  });

  describe('computeYearNetReturnRate 边界（补充）', () => {
    it('totalValues 缺失目标年 → null', () => {
      expect(
        computeYearNetReturnRate({}, ['2023', '2024'], '2024', { '2023': 1000 }),
      ).toBeNull();
    });

    it('totalValues 缺失上一年 → null', () => {
      expect(
        computeYearNetReturnRate({}, ['2023', '2024'], '2024', { '2024': 1000 }),
      ).toBeNull();
    });

    it('上一年值为负 → null（视为无效基准）', () => {
      expect(
        computeYearNetReturnRate({}, ['2023', '2024'], '2024', { '2023': -100, '2024': 1000 }),
      ).toBeNull();
    });
  });
});
